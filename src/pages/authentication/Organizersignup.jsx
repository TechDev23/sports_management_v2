...
const Organizersignup = () => {
  ...
  const addOrganizer = ()=> {
    ...
    setTimeout(() => {
      ...
      //  api call for creating the organizer
  ...
  <div className=" flex items-center justify-center"><p className="text-center font-bold text-3xl border-b-2 w-1/2">Sign up</p></div>
    ...
    <div className="mb-6 space-y-3">
      ...
      <input className="text-xl shadow appearance-none border focus:border-red-500 rounded w-full py-3 px-3 text-gray-700 mb-3 leading-tight focus:outline-orange-500 focus:shadow-outline transition-all" id="password" type="password" placeholder="******************" value={pass} onChange={handlePassChange} />
    </div>
    <div className="flex flex-col items-center justify-between space-y-4">
    <button onClick={addOrganizer} disabled= {isCreateUser} className={`w-3/5 bg-orange-500 hover:bg-orange-700 text-white font-bold py-2 px-4 flex flex-row justify-center rounded-lg focus:outline-none focus:shadow-outline ${isCreateUser && "opacity-50 cursor-not-allowed" }`} type="button">
    {isLoading ? (
    ...
    <p className='font-bold text-sm text-gray-700'>Already have an account? <button onClick={()=> navigate("/organizerlogin")} className="inline-block align-baseline  text-blue-500 hover:text-blue-800" href="#">
    sign in
  </button></p>
