...
const Organizerlogin = () => {
  ...
  <div className=" flex items-center justify-center"><p className="text-center font-bold text-3xl border-b-2 w-1/2">Sign in</p></div>
    ...
    <div className="flex flex-col items-center justify-between space-y-4">
      <button onClick={loginOrganizer} disabled= {isCreateUser} className={`w-3/5 bg-orange-500 hover:bg-orange-700 text-white font-bold py-2 px-4 flex flex-row justify-center rounded-lg focus:outline-none focus:shadow-outline ${isCreateUser && "opacity-50 cursor-not-allowed" }`} type="button">
    {isLoading ? (
    ...
    <p className='font-bold text-sm'>New organizer ? <button onClick={()=> navigate("/organizerregister")} className="inline-block align-baseline  text-blue-500 hover:text-blue-800" href="#">
    sign up
  </button></p>
