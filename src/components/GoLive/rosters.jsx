      ...
      <Card className="mx-auto w-full max-w-[24rem]">
        ...
        <CardFooter className="pt-0">
          <Button
            ...
            fullWidth
          >
            {postScoreState.isLoading ? <Spinner color="amber" />: "Update Score"}
          </Button>
        </CardFooter>
      </Card>
    </Dialog>
...
function WinnerDialog({ open, handleClose, winnerId, setWinnerId, handlePostWinner, postResultState }) {
  return (
    <Dialog
      size="xs"
      open={open}
      ...
      <Card className="mx-auto w-full max-w-[24rem]">
        ...
        <CardFooter className="pt-0">
          <Button
            ...
            fullWidth
          >
            { postResultState.isLoading ? <Spinner color="amber" />: "Set Winner" }
          </Button>
        </CardFooter>
      </Card>
    </Dialog>
...
export default function Rosters() {
  ...
  const handleOpenScoreDialog = () => setScoreDialogOpen(true);
  const handleCloseScoreDialog = () => setScoreDialogOpen(false);
  const handleOpenWinnerDialog = () => setWinnerDialogOpen(true);
  const handleCloseWinnerDialog = () => setWinnerDialogOpen(false);
  ...
  // handle submit score
  const handlePostMatchScore = async({match_id})=>{
                ...
                <>
                  ...
                  >
                    <div>
                      <Input
                        label="team_id1"
                        value={match.team_id1}
                        color="orange"
                      />
                    </div>
                    <div>
                      <p className="p-2 bg-orange-600 text-white rounded-md">
                        {" "}
                        VS{" "}
                      </p>
                    </div>
                  ...
                  <ScoreDialog
                    ...
                    handlePostMatchScore={()=>handlePostMatchScore({match_id:match.id})}
